import {GitHubClientImpl, OriginGitHubCommit, OriginGitHubFork, OriginGitHubRepository, OriginGitHubUser} from './github-client';
  ...
  describe('searchRepositories', () => {
    test('should return a list of github repositories', async () => {
      const MockedFetcher = jest.fn().mockImplementation(() => {
        ...
        return {
          fetch(url: string) {
            expect(url).toBe('https://api.github.com/search/repositories?q=spark&sort=stars&order=desc&per_page=10&page=1');
      ...
      const client = new GitHubClientImpl({numberOfRepositoriesPerPage: 10}, new MockedFetcher());
