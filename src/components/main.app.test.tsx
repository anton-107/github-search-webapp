...
export const buildExampleGitHubRepository: (name: string, id: number) => GitHubRepository = (name: string, id: number) => {
  return {
    id: id,
    name: `${name}-${id}`,
    fullName: '',
...
describe('GitHubSearchApp', () => {
  test('should implement scenario: listing repositories, navigating through pages and clicking on details', async () => {
      ...
      return {
        searchRepositories(query: string, page: number): Promise<GitHubSearchResult> {
          return Promise.resolve({
            totalCount: 215,
            resultsPerPage: 10,
            items: Array.from({length: 10}, (x, i) => buildExampleGitHubRepository(query, i + 10 * (page - 1) ))
          });
        },
    ...
    await waitFor(() => screen.getByText('Show details', {selector: 'a[href="/repository/owner-6/spark-6"]'}));
    
    // navigating to details page:
    fireEvent.click(screen.getByText('Show details', {selector: 'a[href="/repository/owner-6/spark-6"]'}));
    await waitFor(() => screen.getByText('owner-6/spark-6', {selector: 'h3'}));
    expect(screen.getByText('owner-6/spark-6', {selector: 'h3'})).toBeVisible();
    ...
    await waitFor(() => screen.getByText('Show details', {selector: 'a[href="/repository/owner-18/spark-18"]'}));
    expect(screen.getByText('Show details', {selector: 'a[href="/repository/owner-18/spark-18"]'})).toBeVisible();
    ...
    fireEvent.change(screen.getByRole('textbox'), {target: {value: 'kafka'}});
    fireEvent.submit(screen.getByTestId('search-form'));
    await waitFor(() => screen.getByText('Show details', {selector: 'a[href="/repository/owner-1/kafka-1"]'}));
    expect(screen.getByText('Show details', {selector: 'a[href="/repository/owner-1/kafka-1"]'})).toBeVisible();
  });
    ...
    const MockedGitHubClient = jest.fn().mockImplementation(() => {
      return {
        searchRepositories(): Promise<GitHubSearchResult> {
          attemptCounter += 1;
          throw Error(`Could not load repositories (attempt #${attemptCounter})`);
        },
      }
    });
    const mockedClient = new MockedGitHubClient();
    render(<GitHubSearchApp githubClient={mockedClient} />);
    await waitFor(() => screen.getByText('Could not load repositories (attempt #1)'));
    expect(screen.getByText('Could not load repositories (attempt #1)')).toBeVisible();

    fireEvent.click(screen.getByTestId('retry-button'));
    await waitFor(() => screen.getByText('Could not load repositories (attempt #2)'));
    expect(screen.getByText('Could not load repositories (attempt #2)')).toBeVisible();
  });
});