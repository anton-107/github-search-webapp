...
> {
  ...
  public render() {
    const isLoading =
      ...
      this.state.isLastForkInfoLoading ||
      this.state.isLastForkAuthorInfoLoading;
    return (
      <div>
        <div className="columns">
          <div className="column is-one-fifth">
            <Link to="/" className="button" data-testid="back-to-search-button">
              ← Back to search
            </Link>
          </div>
          <div className="column">
            <div className="box">
              ...
              <div className="content">
                ...
                <p>
                  Last 3 commits by:{" "}
                  {this.state.isLastCommitInfoLoading && "Loading..."}{" "}
                  {this.getUniqueAuthorsOfLastCommits()}
                </p>
                <p>
                  Last fork created by:{" "}
                  {this.state.isLastForkInfoLoading && "Loading..."}{" "}
                  {!isLoading &&
                    (this.getLastForkUserLogin() ||
                      "<This repository has not been forked>")}
                </p>
                <p>
                  The owner of the last fork has this in their biography:{" "}
                  {this.state.isLastForkAuthorInfoLoading && "Loading..."}{" "}
                  {!isLoading &&
                    (this.state.mostRecentForkAuthorBio ||
                      "<Their BIO is empty>")}
                </p>
              </div>
            </div>
  ...
  private async loadRepositoryDetails() {
    ...
    const repository = await this.props.githubClient.getRepository(
      this.props.owner,
      this.props.name
    );
  ...
  private async loadLastForkAuthor(userURL: string) {
    ...
    this.setState({
      mostRecentForkAuthorBio: user.bio || null,
      isLastForkAuthorInfoLoading: false,
    });
  }
